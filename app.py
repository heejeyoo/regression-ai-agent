...
if st.button("Predict"):
    ...
    if df.empty:
        st.error("No price data returned for that ticker."); st.stop()

    df = compute_indicators(df)  # robust 1-D safe
    ...
    if X.empty:
        st.error("No rows with complete feature data to predict."); st.stop()

    preds = model.predict(X)

    price_col = next((c for c in ["Adj Close","Close","Open"] if c in df.columns), None)
    chart = pd.DataFrame({"Price": df.loc[X.index, price_col], "Predicted next-day return": preds}, index=X.index)
